...
export function createPageSix() {
  ...
  // Load and process textures - convert black to transparent
  let nyanCatTexture;

  // Load image and convert black pixels to transparent
  const img = new Image();
  img.src = '/sixiChannel1.png';
  img.onload = () => {
    ...
    nyanCatTexture = new THREE.CanvasTexture(canvas);
    nyanCatTexture.minFilter = THREE.LinearFilter;
    nyanCatTexture.magFilter = THREE.LinearFilter;
    nyanCatTexture.needsUpdate = true;

    // Update uniform
    material.uniforms.iChannel1.value = nyanCatTexture;
  };

  // Create audio texture (simulating audio spectrum)
  const audioData = new Uint8Array(256 * 256 * 4);
  const audioTexture = new THREE.DataTexture(audioData, 256, 256, THREE.RGBAFormat);
...
`;
  ...
  const uniforms = {
    ...
    iChannel1: { value: nyanCatTexture }
  };
  ...
  function animate(time) {
    ...
    // Update audio texture with sound simulation
    for (let i = 0; i < 256; i++) {
      const sampleTime = i / 256.0 * 12.0; // 12 seconds as in mainSound
      const modTime = ((sampleTime - 5.0) % 12.0 + 12.0) % 12.0; // mod with positive result
      const soundValue = Math.sin(6.2831 * 440.0 * modTime) * 100.0;
      const envelope = Math.exp(-1.0 * modTime) * Math.min(1.0, modTime);
      const finalValue = Math.abs(soundValue) * envelope;

      for (let j = 0; j < 256; j++) {
        const idx = (j * 256 + i) * 4;
        audioData[idx] = finalValue * 255;     // R
        audioData[idx + 1] = finalValue * 255; // G
        audioData[idx + 2] = finalValue * 255; // B
        audioData[idx + 3] = 255;              // A
      }
    }
    audioTexture.needsUpdate = true;
